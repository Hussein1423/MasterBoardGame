...
export default function BoardMap() {
  ...
  // Group tiles by zone dynamically
  const zonesConfig = Object.values(ZONES)
    .map((zone) => {
      const tilesInZone = TILES_DATA.filter((t) => t.zone.id === zone.id);
      if (tilesInZone.length === 0) return null;
      const minIndex = tilesInZone[0].index;
      const maxIndex = tilesInZone[tilesInZone.length - 1].index;
      return {
        zone,
        tiles: tilesInZone,
        range: `الخانات ${minIndex + 1} - ${maxIndex + 1}`,
        minIndex,
        maxIndex,
      };
    })
  ...
  return (
    <Box sx={{ width: "100%", mb: 3 }}>
      {zonesConfig.map(
        ({ zone, tiles, range, minIndex, maxIndex }, zoneIdx) => {
          // Check if any player is in this zone
          const isPlayerInZone = state.players.some(
            (p) => p.position >= minIndex && p.position <= maxIndex,
          );
            ...
            >
              <Paper
                sx={{
                  ...
                  boxShadow: isPlayerInZone
                    ? `0 0 20px ${zone.color}22`
                    : "none",
                  position: "relative",
                  overflow: "hidden",
                }}
